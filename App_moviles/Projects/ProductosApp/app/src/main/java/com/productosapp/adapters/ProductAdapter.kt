...
class ProductAdapter (
    ...
    override fun onBindViewHolder(holder: ProductHolder, position: Int) {
        if (position < (itemCount - 1)) {
            holder.setItem(productlist[position].item)
            holder.setBrand(productlist[position].brand)
            holder.setModel(productlist[position].model)
            holder.setCostPrice(productlist[position].costprice)
            holder.setSellPrice(productlist[position].sellingprice)
